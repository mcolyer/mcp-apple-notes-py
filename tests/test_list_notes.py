...
class TestListNotes:
    ...
    def test_list_notes_success(self, mock_notes_app, mock_noteslist):
        ...
        # Verify the result
        assert len(result) == 3
        assert result[0] == {"title": "Test Note 1", "id": "x-coredata://test/ICNote/p1"}
        assert result[1] == {"title": "Test Note 2", "id": "x-coredata://test/ICNote/p2"}
        assert result[2] == {"title": "Meeting Notes", "id": "x-coredata://test/ICNote/p3"}
    ...
    def test_list_notes_individual_note_error(self, mock_notes_app):
        ...
        mock_noteslist_with_error.name = ["Good Note", None]  # Second will cause error in zip
