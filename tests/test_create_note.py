...
class TestCreateNote:
    ...
    def test_create_note_success(self, mock_notes_app, mock_account):
        ...
        mock_account.make_note.assert_called_once_with(
            name="Test Note",
            body="<h1>Test Note</h1><p>Content</p>"
        )
...
"""
        ...
        with patch('macnotesapp.NotesApp', return_value=mock_notes_app):
            with patch('main.markdown.Markdown') as mock_markdown:
                mock_md = Mock()
                expected_html = "<h1>My Note</h1><h2>Section 1</h2><ul><li>Item 1</li><li>Item 2</li></ul><p><strong>Bold text</strong> and <em>italic text</em></p><pre><code class=\"language-python\">print(\"Hello, World!\")</code></pre>"
